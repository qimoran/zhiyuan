(function () {
  ...
  async function loadHealth() {
    try {
      ...
      if (rangeEl) rangeEl.textContent = `当前数据库覆盖 ${years} 年数据，包含 ${formatCount(detail.enrollment_plans, "条")}招生计划。`;
      ...
      if (rangeEl) rangeEl.textContent = "数据库统计暂不可用，请确认 Docker 和 MySQL 服务已启动。";
